...
function drawFooter(doc: jsPDF, documentId: string) {
  ...
  for (let p = 1; p <= pageCount; p++) {
    ...
    doc.text(
      `NGAO  •  Verified Proof of Service  •  Document ID: ${documentId}  •  Page ${p} of ${pageCount}`,
      40,
      pageH - 28,
    );
...
export function generateReportPDF(input: ReportInput) {
  ...
  const documentId = `NGAO-${generatedAt.getTime().toString(36).toUpperCase()}`;
  ...
  y += 20;
  doc.text(`Verified by:  ${input.verifiedBy ?? "NGAO Quality Assurance"}`, 40, y);
